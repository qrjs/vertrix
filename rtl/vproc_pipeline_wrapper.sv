...
`endif
    ...
    // Note: Operands using the alternative counter (e.g., stride indices) are fetched
    // out of sync with other operands. Current implementation uses fixed stage delays
    // to avoid vreg read port collisions. The pipeline waits for alt count completion
    // before accepting the next instruction, preventing race conditions.
