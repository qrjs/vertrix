...
static float logits[FC3_OUT] NOINIT;

int main(void) {
    conv3x3_fp32(test_sample_0, INPUT_H, INPUT_W, INPUT_C, conv1_weight, CONV1_OUT_C, conv1_out);
    relu_fp32_inplace(conv1_out, 28 * 28 * CONV1_OUT_C);
    maxpool2x2_fp32(conv1_out, 28, 28, CONV1_OUT_C, pool1_out);
    asm volatile("fence" ::: "memory");

    conv3x3_fp32(pool1_out, POOL1_OUT_H, POOL1_OUT_W, CONV1_OUT_C, conv2_weight, CONV2_OUT_C, conv2_out);
    ...
    fc_fp32(fc2_out, FC2_OUT, fc3_weight, FC3_OUT, logits);
    asm volatile("fence" ::: "memory");

    result[0] = argmax_fp32(logits, FC3_OUT);
