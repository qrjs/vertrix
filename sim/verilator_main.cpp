...
int main(int argc, char **argv) {
    ...
    fprintf(fcsv, "rst_ni;imem_req;imem_addr;dmem_req;dmem_addr;pend_vreg_wr_map_o;\n");
    ...
    // 指令内存延迟队列
    int64_t *imem_rvalid_queue = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    int64_t *imem_rdata_queue  = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    int64_t *imem_err_queue    = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    // 数据内存延迟队列
    int64_t *dmem_rvalid_queue = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    int64_t *dmem_rdata_queue  = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    int64_t *dmem_err_queue    = (int64_t *)malloc(sizeof(int64_t) * mem_latency);
    ...
    while (getline(&line, &line_sz, fprogs) > 0) {
        ...
        // simulate program execution
        {
            ...
            for (i = 0; i < mem_latency; i++) {
                imem_rvalid_queue[i] = 0;
                imem_rdata_queue[i] = 0;
                imem_err_queue[i] = 0;
                dmem_rvalid_queue[i] = 0;
                dmem_rdata_queue[i] = 0;
                dmem_err_queue[i] = 0;
            }
            top->imem_rvalid_i = 0;
            top->dmem_rvalid_i = 0;
            ...
            while (end_cnt < extra_cycles) {
                ...
                unsigned dmem_addr  = top->dmem_addr_o & ~(mem_w/8-1);
                dmem_rdata_queue[0] = 0;
                if (dmem_valid) {
                    // 写入内存
                    if (top->dmem_req_o && top->dmem_we_o) {
                        for (i = 0; i < mem_w / 8; i++) {
                            if ((top->dmem_be_o & (1<<i))) {
                                mem[dmem_addr+i] = top->dmem_wdata_o >> (i*8);
                            }
                        }
                    }
                    // 读取内存
                    for (i = 0; i < mem_w / 8; i++) {
                        dmem_rdata_queue[0] |= ((int64_t)mem[dmem_addr+i]) << (i*8);
                    }
                }
                ...
                dmem_err_queue   [0] = !dmem_valid;

                int imem_req_o_tmp = top->imem_req_o;
                int dmem_req_o_tmp = top->dmem_req_o;
                ...
                // 返回指令内存响应
                top->imem_rvalid_i = imem_rvalid_queue[mem_latency-1];
                top->imem_rdata_i  = imem_rdata_queue [mem_latency-1];
                top->imem_err_i    = imem_err_queue   [mem_latency-1];
                // 返回数据内存响应
                top->dmem_rvalid_i = dmem_rvalid_queue[mem_latency-1];
                top->dmem_rdata_i  = dmem_rdata_queue [mem_latency-1];
                top->dmem_err_i    = dmem_err_queue   [mem_latency-1];
                top->eval();
    ...
    free(line);
    free(mem);
    free(imem_rvalid_queue);
    free(imem_rdata_queue);
    free(imem_err_queue);
    free(dmem_rvalid_queue);
    free(dmem_rdata_queue);
    free(dmem_err_queue);
    fclose(fcsv);
    fclose(fprogs);
    return 0;
...
static void log_cycle(Vvproc_top *top, VerilatedTrace_t *tfp, FILE *fcsv) {
    fprintf(fcsv, "%d;%d;%08X;%d;%08X;%08X;\n",
            top->rst_ni, top->imem_req_o, top->imem_addr_o, 
            top->dmem_req_o, top->dmem_addr_o, top->pend_vreg_wr_map_o);
    main_time++;
#if defined(TRACE_VCD) || defined(TRACE_FST)
    if (tfp != NULL)
        tfp->dump(main_time);
