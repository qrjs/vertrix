...
if {[string first "ibex" $core_dir] != -1} {
    ...
    set core_rtl_dir "$vproc_dir/rtl"
    set vendor_dir "$vproc_dir/vendor/lowrisc_ip"
    lappend src_list "$core_rtl_dir/ibex_pkg.sv"
    lappend src_list "$core_rtl_dir/ibex_tracer_pkg.sv"
    lappend src_list "$vendor_dir/ip/prim/rtl/prim_ram_1p_pkg.sv"
    foreach file [glob -type f $core_rtl_dir/ibex_*.sv] {
        lappend src_list $file
    }
    lappend src_list "$vproc_dir/syn/rtl/prim_clock_gating.v"
    set dv_utils "$vendor_dir/dv/sv/dv_utils/"
    set prim "$vendor_dir/ip/prim/rtl/"
} else {
    puts "ERROR: Only Ibex is supported as main core."
    exit 2
}
